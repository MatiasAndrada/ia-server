...
> ⚠️ **Este documento es sobre el sistema legacy de `src/agents/` (config-based, un solo agente `waitlist`). No es el orquestador que atiende WhatsApp.** Ese es otro sistema, `src/agent/` (singular, sin `s`) — un orquestador con tool-calling que lleva **toda** la conversación de WhatsApp; no pasa por el `waitlist` agent descripto acá. Ver la sección "Flujo de Procesamiento" del [README](../README.md#arquitectura) para el detalle.
>
> **Qué maneja el `waitlist` agent de este documento, y qué no:** el flujo de reserva por WhatsApp lo lleva el orquestador de `src/agent/`, no este agente. El `waitlist` agent se usa únicamente como endpoint standalone (`POST /api/agents/waitlist/chat`, más abajo) para integraciones externas o pruebas, sin pasar por WhatsApp.

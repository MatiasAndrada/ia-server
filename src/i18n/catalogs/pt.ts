...
export const ptCatalog: MessageCatalog = {
  ...
  closedNoAvailability(): string {
    return `❌ Estamos fechados neste momento e não encontrei disponibilidade nos próximos 7 dias.`;
  },
  ...
  outOfWindowPrefix(): string {
    return `Por enquanto só posso aceitar reservas dentro dos próximos 7 dias. `;
  },
  ...
  weekdayDayMismatchPrompt(weekdayLabel: string, requestedDayNumber: number, nearestLabel: string): string {
    return `Por enquanto só posso aceitar reservas para *esta semana e a próxima*, então não consigo agendar para *${weekdayLabel} dia ${requestedDayNumber}*.\n\nVocê quer que seja *${nearestLabel}* (a próxima ${weekdayLabel}) no lugar?\n\nResponda *sim* ou *não*.`;
  },
  ...
  reservationOutOfWindow(businessName: string): string {
    return `Olá 😊 No “${businessName}” por enquanto só posso aceitar reservas dentro dos próximos 7 dias. Você quer escolher um dia mais próximo?`;
  },
